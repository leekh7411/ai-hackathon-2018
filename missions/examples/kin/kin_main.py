...
def text_input_binary_clf_RNN_CNN_model(input_length=400):
    model = Sequential()
    model.add(Embedding(10000, 128, input_length=input_length))
...
if __name__ == '__main__':
    ...
    if config.mode == 'train':
        ...
        if dataset_len % config.batch != 0:
            one_batch_size += 1
        # epoch마다 학습을 수행합니다.
        test_x = dataset.queries[0]
        test_y = dataset.labels[0]

        hist = model.fit(dataset.queries, dataset.labels, epochs=config.epochs, batch_size=100)
